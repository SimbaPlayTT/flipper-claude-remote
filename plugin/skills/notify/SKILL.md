...
The bridge listens on a Unix socket on macOS/Linux and a loopback TCP port on
Windows; the helper script below picks the right endpoint automatically.

## Notify (sound + vibration + two-line text)

```bash
python "${CLAUDE_PLUGIN_ROOT}/scripts/flipper-notify.py" SOUND VIBRO "TITLE" "SUBTITLE"
```

- `SOUND` — one of the sound names below
- `VIBRO` — `true` to vibrate, `false` for silent
- `TITLE` / `SUBTITLE` — up to 20 characters each; pass `""` if not needed
...
Task completed:
```bash
python "${CLAUDE_PLUGIN_ROOT}/scripts/flipper-notify.py" success true "Done" "Tests passed"
```

Error occurred:
```bash
python "${CLAUDE_PLUGIN_ROOT}/scripts/flipper-notify.py" error true "Build failed" ""
```

Silent status update:
```bash
python "${CLAUDE_PLUGIN_ROOT}/scripts/flipper-notify.py" alert false "Thinking..." ""
```
...
- If the bridge is not running, the script exits silently with no error shown to the user
