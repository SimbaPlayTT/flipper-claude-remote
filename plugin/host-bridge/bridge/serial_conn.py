...
class SerialConnection:
    ...
    async def connect(self) -> bool:
        # A transport failure (e.g. the Bluetooth radio is switched off)
        # must not crash the daemon: report "not connected" and let the
        # reconnect loop keep trying.
        try:
            ok = await self._transport.connect()
        except Exception as e:
            log.error("Transport connect error: %s", e)
            ok = False
        if ok:
            self.connected = True
        return ok
