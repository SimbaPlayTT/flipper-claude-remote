...
# ---------------------------------------------------------------------------
# Windows voice typing backend (Win+H)
# ---------------------------------------------------------------------------

class WindowsDictationBackend(DictationBackend):
    """Windows 10/11 voice typing, toggled with the Win+H shortcut.

    start() opens the voice-typing flyout over the focused window; the ESC
    the daemon sends on stop dismisses it, so stop() needs no extra work.
    There is no reliable way to query the flyout's state, so is_active()
    stays True and the user toggles off manually (same contract as the
    custom backend without a check command).
    """

    async def start(self) -> None:
        from . import winput  # Windows-only module

        winput.send_vk(winput.vk_for_char("h"), (winput.VK_LWIN,), "voice(Win+H)")

    # stop() is intentionally left as no-op: the daemon sends ESC which
    # dismisses the Windows voice-typing flyout.
...
def create_backend() -> DictationBackend:
    ...
    if backend_type == "windows":
        log.info("Dictation backend: Windows voice typing (Win+H)")
        return WindowsDictationBackend()
