...
# Abstract key names → Windows virtual-key attribute names in bridge.winput
_WIN_KEY_NAMES: dict[str, str] = {
    "return":    "VK_RETURN",
    "escape":    "VK_ESCAPE",
    "down":      "VK_DOWN",
    "up":        "VK_UP",
...
# macOS keycode → Windows virtual-key code (only codes actually used by the
# bridge: letters c/e/o, Return, Tab, Space, Backspace, Escape, PgUp/PgDn, Down)
...
# macOS modifier phrase → Windows virtual-key code
_MACOS_MOD_TO_VK: dict[str, int] = {
    "control down": 0x11,  # VK_CONTROL
    "shift down":   0x10,  # VK_SHIFT
    "option down":  0x12,  # VK_MENU (Alt)
    "command down": 0x5B,  # VK_LWIN
}


class WindowsInputBackend(InputBackend):
    """Windows input backend using SendInput via ctypes (bridge.winput).

    Window targeting uses the ``window_id`` captured by the session-target
    hook (the terminal's HWND at prompt time). Without a window ID keystrokes
    go to the current foreground window — same behaviour as the Linux backend
    without an X11 WINDOWID.
    """

    def __init__(self) -> None:
        from . import winput  # Windows-only module
        self._winput = winput
        self._target: InputTarget | None = None
    ...
    def set_target(self, target: dict[str, str] | None) -> None:
        ...
        log.info(
            "Input target set: %s",
            self._target.describe() if self._target else "foreground window",
        )

    async def _focus(self) -> None:
        if not (self._target and self._target.window_id):
            return
        try:
            hwnd = int(self._target.window_id)
        except ValueError:
            return
        # focus_hwnd polls up to ~1s; keep it off the event loop.
        await asyncio.get_running_loop().run_in_executor(
            None, self._winput.focus_hwnd, hwnd
        )

    async def send_ctrl_c(self) -> None:
        await self._focus()
        self._winput.send_vk(0x43, (0x11,), "Ctrl+C")  # Ctrl + 'C'

    async def send_keystroke(self, key: str) -> None:
        vk = getattr(self._winput, _WIN_KEY_NAMES.get(key, ""), None)
        if vk is None:
            log.warning("keystroke(%s): no Windows key mapping", key)
            return
        await self._focus()
        self._winput.send_vk(vk, (), f"keystroke({key})")

    async def send_text(self, text: str) -> None:
        await self._focus()
        self._winput.send_unicode(text, "send_text")
        self._winput.send_vk(self._winput.VK_RETURN, (), "send_text")

    async def send_chars(self, text: str, *, focus: bool = True) -> None:
        if focus:
            await self._focus()
        self._winput.send_unicode(text, "send_chars")

    async def send_modified_keystroke(self, key_code: int, modifiers: str) -> None:
        vk = _MACOS_KEYCODE_TO_VK.get(key_code)
        if vk is None:
            log.warning("keystroke(code=%d): no Windows key mapping", key_code)
            return
        mod_vk = _MACOS_MOD_TO_VK.get(modifiers.lower().strip())
        mods = (mod_vk,) if mod_vk else ()
        await self._focus()
        self._winput.send_vk(vk, mods, f"keystroke(code={key_code}, mod={modifiers})")
...
def create_backend() -> InputBackend:
    ...
    if sys.platform == "win32":
        log.info("Input backend: SendInput (Windows)")
